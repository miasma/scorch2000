...
public class MainToolbar extends sWindow {
	...
	if( code == KeyEvent.VK_F2 )
		...
		StatsWindow sw = new StatsWindow
		    (StatsWindow.IG,applet);
		sw.display();
		return;
	    }

	if( code == KeyEvent.VK_F3 && !(applet).isGuest() )
	    {
		NewUser nu = 
		    new NewUser		
	...
	if( code == KeyEvent.VK_F5 && 
	    ...
	    inventory.isEnabled())
	    {
		wndInventory = new Inventory(applet);
	...
	if( code == KeyEvent.VK_LEFT)
	    {
		updateAngleLabel((applet).changeAngle(scale));
		return;
	    }
	...
	if( code == KeyEvent.VK_HOME)
	    {
		updateAngleLabel((applet).changeAngle(10));
		return;
	    }
	...
	if( code == KeyEvent.VK_UP || key == '+' )
	    {
		updatePowerLabel((applet).changePower(scale));
		return;
	    }
	...
	if( code == KeyEvent.VK_PAGE_UP )
	    {
		updatePowerLabel((applet).changePower(10));
		return;
	    }
    ...
    public boolean handleEvent(Event evt) {
        ...
        if (evt.id == Event.ACTION_EVENT) {
            ...
            if (evt.target == left) {
                updateAngleLabel((applet).changeAngle(3));
                return true;
            }
            if (evt.target == right) {
                updateAngleLabel
                        ((applet).changeAngle(-3));
                return true;
            }
            if (evt.target == up) {
                updatePowerLabel((applet).changePower(5));
                return true;
            }
            if (evt.target == down) {
                updatePowerLabel
                        ((applet).changePower(-5));
                return true;
            }
        ...
        if (evt.id == Event.KEY_ACTION || evt.id == Event.KEY_PRESS) {
            ...
            if (evt.key == Event.F3 && !(applet).isGuest()) {
                NewUser nu =
                        new NewUser
                                (applet.getMyPlayer().getName(),
                                        applet,
